...
TEST_TYPES = [
    'OVRPB', 'OVRPL', 'OVRPTW', 'VRPBL', 'VRPBTW',
]
...
logger_params = {
    'log_file': {
        'desc': f"v4_typesplit5_size50_1000ep_seed{SEED}",
        'filename': 'run_log'
    }
}
...
def _check_model():
    ...
    if os.path.exists(p): logger.info(f"✓ Dedicated 50-node model: {p}")
...
def _print_config():
    logger = logging.getLogger('root')
    logger.info('='*80)
    logger.info('CGPRL — 5-TRAIN + 5-TEST TYPE SPLIT, SCALE-50 (1000 ep)')
    logger.info(f'  Train types ({len(TRAIN_TYPES)}): {TRAIN_TYPES}')
    logger.info(f'  Test  types ({len(TEST_TYPES)}): {TEST_TYPES}')
